...
const COLORS = ['#ff99c8', '#d0f4de', '#a9def9', '#e4c1f9']; // 可用顏色陣列
...
function draw() { // p5.js 每幀執行的繪製函式
  background('#cdb4db'); // 清除畫面並填背景色
  noStroke(); // 關閉描邊

  // 左上顯示固定編號 414730522（黑色、字型大小 32px）
  push();
  textSize(32); // 左上文字大小 32px
  fill(0); // 黑色文字
  textAlign(LEFT, TOP);
  text('414730522', 16, 12);
  pop();
...
function createAudioStartButton() {
  ...
  return btn; // 回傳按鈕參考（如需）
} // 結束 createAudioStartButton()
